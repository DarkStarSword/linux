					       ...
					       pci_channel_state_t state)
{
	...
	printk("%s: state: %s. %s a reflash\n", __func__,
	       ((state == pci_channel_io_normal) ? "normal" :
		((state == pci_channel_io_frozen) ? "frozen" :
		 ((state == pci_channel_io_perm_failure) ? "failed" : "unknown"))),
	       (adapter->perst_loads_image ? "Doing" : "Not doing"));
	...
	if (adapter->perst_loads_image && !adapter->perst_same_image) {
		...
		printk("cxl: reflashing, so opting out of EEH!\n");
