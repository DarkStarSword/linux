...
static void switch_card_to_cxl(struct work_struct *work)
{
	...
	if (switch_work->mode == CXL_BIMODE_CXL) {
		...
		pnv_cxl_enable_phb_kernel2_api(dev, true);
	} else {
		dev_info(&dev->dev, "Switching card to PCI mode\n");
		val &= ~CXL_VSEC_PROTOCOL_ENABLE;
		pnv_cxl_enable_phb_kernel2_api(dev, false);
	}
...
static int cxl_configure_adapter(struct cxl *adapter, struct pci_dev *dev)
{
	 ...
	 */
	if ((rc = cxl_check_and_switch_mode(dev, CXL_BIMODE_CXL, 0)))
		return rc;
...
#if 1
	/*
	 * Disable snooping tlb traffic. Theory is that this might be necessary
	 * for XSL based devices that use CAPP DMA mode to avoid an EEH, but
	 * didn't work - at least not by itself.
	 */
	WARN_ON(pnv_phb_to_cxl_mode(pdev, OPAL_PHB_CAPI_MODE_SNOOP_OFF));
#endif
...
static const struct cxl_service_layer_ops xsl_ops = {
	...
	.min_pe = 1, /* Workaround for Mellanox CX4 firmware bug */
};
